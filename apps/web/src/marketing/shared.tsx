...
/**
 * <picture> wrapper for the marketing PNGs that ship with a WebP sibling
 * (generated by `node scripts/convert-marketing-webp.mjs`). Browsers that
 * understand `image/webp` (all evergreens + Safari ≥14) pick the WebP and
 * save us ~30× bytes; everything else falls through to the PNG.
 *
 * Defaults to `loading="lazy"` because every caller except the Landing hero
 * sits below the fold — for the hero, pass `eager` so the LCP image isn't
 * deferred. `decoding="async"` is always on; it lets the browser keep
 * painting while the image decodes off-thread.
 *
 * Pass a `/marketing/foo.png` `src`; we derive the `.webp` sibling. If you
 * pass a non-`.png`, we just emit a plain <img> with the same attrs.
 */
export function MarketingPicture({
  ...
  src: string;
  alt: string;
  /** True for above-the-fold images (the Landing hero). Default false (lazy-load). */
  eager?: boolean;
  style?: CSSProperties;
  className?: string;
  ...
  const webp = isPng ? src.replace(/\.png$/i, '.webp') : null;
  const imgProps = {
    src,
    alt,
    style,
    ...
    // The hero is the LCP candidate; hint the browser to prioritise it.
    // `fetchPriority` (camelCase) is the React 18.3 prop; on older React
    // versions the runtime would warn but still pass it through.
